...
df = pd.read_csv('./HW4-2/optdigits.tra', sep=',', header=None) #reading the data by using Pandas library

X_training = np.array(df.values)[:,:64] #getting the first 64 fields to form the feature data for training
y_training = np.array(df.values)[:,-1]  #getting the last field to form the class label for training

df = pd.read_csv('./HW4-2/optdigits.tes', sep=',', header=None) #reading the data by using Pandas library
...
for w in n: #iterates over n
    ...
    for b in r: #iterates over r
        ...
        for a in range(2): #iterates over the algorithms
            ...
            if a == 0 and localAccuracy > accuracy[0]:
               accuracy[0] = localAccuracy
               print(f"Highest Perceptron accuracy so far: {localAccuracy:.2%}, Parameters: learning rate={w}, shuffle={b}")
            elif a == 1 and localAccuracy > accuracy[1]:
               accuracy[1] = localAccuracy  
               print(f"Highest MLP accuracy so far: {localAccuracy:.2%}, Parameters: learning rate={w}, shuffle={b}")
